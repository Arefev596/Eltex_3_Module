...
int main(int argc, char **argv) {
	int port = atoi (argv[2]); ////////// считываем порт для подключения UDP
	...
	if(argc != 3){
		printf("Usage: <IP address> and <PORT (49152-65535)\n");
		exit(1);
	}
	/* Создаем UDP сокет */
	...
	servaddr.sin_port = htons(port); ///////////вместо 0 ставим переменную с портом
	// 10 слайд в лекции
	...
	while(1){
		...
		/* Ожидаем ответа и читаем его. Максимальная допустимая длина датаграммы – 1000 символов,
		адрес отправителя нам не нужен */
		if((n = recvfrom(sockfd, recvline, 1000, 0, (struct sockaddr *) &servaddr, &len)) < 0){
